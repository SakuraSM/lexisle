  ...
  return (
    <div className="page notes-page">
      ...
      <div className="notes-layout">
        ...
        <section className="note-editor open-panel"><div className="editor-toolbar"><span>{selected ? "编辑笔记" : "新笔记"}</span>{selected ? <button className="danger-link" type="button" onClick={remove}><TrashIcon />删除</button> : null}</div><input className="note-title-input" value={draft.title} onChange={(event) => setDraft((value) => ({ ...value, title: event.target.value }))} placeholder="笔记标题" /><textarea value={draft.body} onChange={(event) => setDraft((value) => ({ ...value, body: event.target.value }))} placeholder="用自己的话记录理解、摘录例句或提出问题……" /><label className="tag-input"><span>标签</span><input value={draft.tags.join("、")} onChange={(event) => setDraft((value) => ({ ...value, tags: event.target.value.split(/[、,，]/).map((tag) => tag.trim()).filter(Boolean) }))} placeholder="睡眠、记忆" /></label><div className="editor-footer"><span>{draft.body.length} 字</span><button className="primary-button" type="button" onClick={save}>保存笔记</button></div></section>
      </div>
    </div>
  );
}
