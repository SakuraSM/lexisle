...
import { isDue, speak } from "../lib/learning.js";
...
export function ReviewPage({ state, actions, notify }) {
  ...
  return (
    <div className="page review-page">
      <PageHeader title="复习" description="根据记忆曲线，在即将遗忘时巩固单词。" action={<div className="review-count"><ReloadIcon /> 今日已复习 <strong>{state.plans[new Date().toISOString().slice(0, 10)]?.reviewDone || 0}</strong> 个</div>} />
      <div className="review-layout">
        <section className="review-stage open-panel">
          ...
          {current ? <div className={`flashcard ${revealed ? "is-revealed" : ""}`}>
            <button className="sound-circle" type="button" onClick={() => speak(current.word)}><SpeakerLoudIcon /></button>
          ...
          {current && revealed ? <div className="grade-row">{Object.entries(resultLabels).map(([result, label]) => <button key={result} type="button" onClick={() => grade(result)}><strong>{label}</strong><span>{result === "again" ? "1 天" : result === "hard" ? "2 天" : result === "good" ? "4 天" : "7 天"}</span></button>)}</div> : null}
        </section>
        <aside className="review-sidebar open-panel"><h2>今日复习</h2><div className="review-ring"><strong>{state.plans[new Date().toISOString().slice(0, 10)]?.reviewDone || 0}</strong><span>已完成</span></div><ul><li><CheckCircledIcon /><span>到期单词</span><strong>{initialQueue.length}</strong></li><li><CheckCircledIcon /><span>掌握词汇</span><strong>{state.vocabulary.filter((item) => item.status === "mastered").length}</strong></li><li><CheckCircledIcon /><span>平均记忆强度</span><strong>{Math.round(state.vocabulary.reduce((sum, item) => sum + Math.min(100, item.repetition * 16), 0) / state.vocabulary.length)}%</strong></li></ul></aside>
      </div>
    </div>
  );
}
