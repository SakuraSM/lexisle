...
export function TodayPage({ state, actions, navigate, openArticle, notify }) {
  ...
  const tasks = [
    { label: "阅读文章", value: plan.readingDone, target: plan.readingTarget, icon: ReaderIcon, tone: "primary", action: () => openArticle(current.id) },
  ...
  const completion = Math.round(tasks.reduce((sum, task) => sum + Math.min(1, task.value / task.target), 0) / 3 * 100);
  ...
  return (
    <div className="page today-page">
      ...
      <section className="today-overview">
        <div className="today-score"><span>今日完成度</span><strong>{completion}%</strong><ProgressMeter value={completion} max={100} /><p><LightningBoltIcon /> 已连续学习 {state.streak} 天</p></div>
      ...
      {editing ? <div className="inline-modal-backdrop" onMouseDown={() => setEditing(false)}><form className="plan-dialog" onMouseDown={(event) => event.stopPropagation()} onSubmit={(event) => { event.preventDefault(); actions.updatePlan(draftPlan); setEditing(false); notify("今日计划已更新"); }}><h2>编辑今日计划</h2><p>目标应当有一点挑战，但每天都能完成。</p>{[["readingTarget", "阅读文章", "篇"], ["wordTarget", "学习生词", "个"], ["reviewTarget", "复习单词", "个"]].map(([key, label, unit]) => <label key={key}><span>{label}</span><div><input type="number" min="1" max="50" value={draftPlan[key]} onChange={(event) => setDraftPlan((value) => ({ ...value, [key]: Number(event.target.value) }))} /><em>{unit}</em></div></label>)}<div className="dialog-actions"><button className="secondary-button" type="button" onClick={() => setEditing(false)}>取消</button><button className="primary-button" type="submit">保存计划</button></div></form></div> : null}
    </div>
  );
}
